...
Fleet UI는 [Lucide React Native](https://lucide.dev/guide/packages/lucide-react-native)를 사용하여 아이콘을 제공합니다.
...
### 사용 (Component as Prop 패턴)

필요한 아이콘만 import하여 `icon` prop으로 전달하는 방식을 사용합니다. 이는 Tree-shaking을 지원하여 앱 크기를 최적화합니다.
...
function MyComponent() {
  return (
    <>
      {/* 기본 사용 */}
      <Icon icon={Heart} accessibilityLabel="Like" />
...
1. **Tree-shaking**: 필요한 아이콘만 번들에 포함
2. **일관된 스타일**: Fleet UI의 ColorScheme 및 Size 토큰 자동 적용
3. **접근성**: `accessibilityLabel` 필수값 강제 및 자동 `role="image"` 적용
4. **타입 안전성**: TypeScript가 올바른 아이콘 컴포넌트인지 검증
