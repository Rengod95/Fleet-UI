...
export function CodeBlock({
  ...
  const copyText = useMemo(() => {
    ...
    function extract(node: unknown): string {
      ...
      if (isValidElement(node)) {
        return extract((node.props as { children?: unknown }).children);
      }
      return '';
    }
  ...
  return (
    <div className={cn('group relative', className)}>
      ...
      {html ? (
        <div
          // Generated by internal build script (Shiki). Safe to render as HTML.
          // biome-ignore lint/security/noDangerouslySetInnerHtml: internal-only highlighted HTML
          dangerouslySetInnerHTML={{ __html: html }}
        />
      ) : (
        <pre className="overflow-x-auto rounded-xl bg-muted p-4 text-xs text-foreground">
          {children ? children : <code className="font-mono">{normalized}</code>}
        </pre>
      )}
    </div>
  );
